...
use rayon::prelude::*;
use crate::config::{TilerConfig, Projection};
        ...
        for row in 0..actual_cube_size {
            for col in 0..actual_cube_size {
                ...
                let pixel = if is_outside {
                    ...
                    sample_bilinear(src_image, src_x, src_y, config.partial_config.haov >= 360.0, bg_color)
                };
...
fn sample_bilinear(
    ...
    let y0 = y.floor();
    let x1 = if wrap_x { (x0 + 1.0) % w_f } else { (x0 + 1.0).min(w_f - 1.0) };
